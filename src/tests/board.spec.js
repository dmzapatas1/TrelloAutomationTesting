...
describe('Board suite', () => {
  ...
  describe('Card and list suite', () => {
    it('Successfully creates a list', async () => {
      const newListTitle = helpers.Utility.createRandomItem('List test_');
      await boardPage.createNewList(newListTitle);
      await browser.waitUntil(async () => {
        const lastItemList = await helpers.Utility.getTextLastItem(boardPage.listTitles());
    ...
    it('Successfully creates a card', async () => {
      ...
      await browser.waitUntil(async () => {
        const lastItemList = await helpers.Utility.getTextLastItem(boardPage.listCardsToDo());
    ...
    it('Should delete first card of the first list', async () => {
      const titleFirstCard = await boardPage.listCardsToDo()[0].getText();
      await boardPage.deleteCard(0);
      await browser.waitUntil(async () => {
        const isTitleFirstCard = await boardPage.listCardsToDo().some(title => title === titleFirstCard);
  ...
  describe.skip('Share board suite', () => {
    it('Successfully shares board with another member', async () => {
      await boardPage.addMember('dmzapatas10@gmail.com');
      const titleAttributeValue = await boardPage.memberList()[1].getAttribute('title');
    ...
    it('Displays error when adding a new member', async () => {
      ...
      const errorShareBoardExists = await boardPage.errorShareBoard().isExisting();
